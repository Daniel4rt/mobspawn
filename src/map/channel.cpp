...
int channel_send(struct Channel *channel, struct map_session_data *sd, const char *msg) {
	...
	// Desactiva el uso de Main y Trade durante WoE
	if((agit_flag || agit2_flag || agit3_flag) && battle_config.disable_main_trade_woe) {
		clif_messagecolor(&sd->bl,color_table[COLOR_RED],"Lo siento, pero no puedes usar este canal durante WoE.",false,SELF);
		return -2;
	}
