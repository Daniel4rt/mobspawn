...
int pc_get_group_id(struct map_session_data *sd) {
	return pc_isPremium(sd) ? battle_config.vip_group_level : sd->group_id;
}
...
bool pc_isPremium(struct map_session_data *sd)
{
	...
	return ( battle_config.vip_group_level && sd->Premium_Tick > (int)time(NULL) );//aqui es donde tiro el error. 
}
...
void pc_Premium_validate(struct map_session_data *sd)
{
	...
	if( tick > 0 )
	{
		...
		else
		{
			...
			sd->group_id = battle_config.vip_group_level;
			pc_group_pc_load(sd);
		}
...
* @param n
*/
int pc_equippoint_sub2(struct map_session_data *sd,struct item_data* id,int n){
	int ep = 0, char_id = 0;
...
int pc_equippoint(struct map_session_data *sd,int n){
	...
	return pc_equippoint_sub2(sd,sd->inventory_data[n],n);
}
...
uint8 pc_isequip(struct map_session_data *sd,int n)
{
	...
	if (sd->sc.count) {
		...
		if (sd->sc.data[SC_SPIRIT] && sd->sc.data[SC_SPIRIT]->val2 == SL_SUPERNOVICE) {
			...
			if (sd->status.base_level > 96 && item->equip & EQP_ARMS && item->type == IT_WEAPON)//AHORA SE PUEDEN COLOCAR TODAS LAS ARMAS DE TODOS LOS NIVELES.
...
// (^~_~^) LGP End
	...
	sd->Premium_Tick = pc_readaccountreg(sd,add_str(PREMIUM_TICK_EAMOD));

	sd->bg_team = pc_readglobalreg(sd,add_str(BATTLEGROUND_TEAM));
...
short pc_search_inventory(struct map_session_data *sd, unsigned short nameid) {
	...
	if( nameid )
	{
		if( map_bg_items(sd->bl.m,battle_config.bg_items_on_pvp,battle_config.bg_woe_switch) && battle_config.bg_reserved_char_id )
		{ // Battleground Items
	...
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		...
		{
			if( !map_bg_items(sd->bl.m,battle_config.bg_items_on_pvp,battle_config.bg_woe_switch) && battle_config.bg_reserved_char_id && battle_config.bg_reserved_char_id == char_id )
				continue;
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	{ // Do not allow use BG and WoE Items on invalid maps
		char_id = MakeDWord(item.card[2],item.card[3]);
		if( battle_config.bg_reserved_char_id && char_id == battle_config.bg_reserved_char_id && !map_bg_items(sd->bl.m,battle_config.bg_items_on_pvp,battle_config.bg_woe_switch) )
			return 0;
	...
	/* on restricted maps the item is consumed but the effect is not used [DanielArt] */
	if (!pc_has_permission(sd,PC_PERM_ITEM_UNCONDITIONAL) && (itemdb_isNoEquip(id,sd->bl.m) || !itemdb_isOnly(id,sd->bl.m))) {
	...
	pc_setreg(sd, add_str("@itemused_id"), sd->itemid);
...
int pc_steal_item(struct map_session_data *sd,struct block_list *bl, uint16 skill_lv)
{
	...
	if(md->db->dropitem[i].p<=battle_config.rare_drop_announce) {
		...
		sprintf (message, msg_txt(sd,542), md->db->jname, i_data->jname, (float)md->db->dropitem[i].p/100);
...
enum e_setpos pc_setpos(struct map_session_data* sd, unsigned short mapindex, int x, int y, clr_type clrtype)
{
	...
	if( sd->state.changemap ) { // Misc map-changing settings
		int i;
		unsigned short instance_id = map_getmapdata(sd->bl.m)->instance_id;//https://github.com/rathena/rathena/commit/7d8530e3b1f679c6c5ad54bbce97c41d31c4b6b5
		...
		if (sd->state.vending && mapdata->flag[MF_NOVENDING] && itemdb_is_merchant(sd) ) {
	...
	if (sd->state.vending && map_getcell(m,x,y,CELL_CHKNOVENDING) && itemdb_is_merchant(sd) ) {
	...
	//[Easycore]
	if (!sd->state.changemap && sd->state.last_teleport && !map_flag_vs2(sd->bl.m))
		clif_viewpoint(sd,sd->bl.id,1,sd->bl.x,sd->bl.y,1,0xFF0000);
...
int pc_get_skillcooldown(struct map_session_data *sd, uint16 skill_id, uint16 skill_lv) {
	...
	int mindelay = skill_get_mindelay(skill_id);
...
static void pc_calcexp(struct map_session_data *sd, unsigned int *base_exp, unsigned int *job_exp, struct block_list *src)
{
	...
	if (src) {
		...
		if (src && src->type == BL_MOB && pc_isPremium(sd)) { // EXP bonus for VIP player
			vip_bonus_base = battle_config.base_exp_vip_bonus;
			vip_bonus_job = battle_config.job_exp_vip_bonus;
		}
	}
	...
	if (sd->sc.data[SC_EXPBOOST]) {
		...
		if (battle_config.vip_bm_increase && pc_isPremium(sd)) // Increase Battle Manual EXP rate for VIP
...
int pc_dead(struct map_session_data *sd,struct block_list *src, uint16 skill_id)
{
	...
	if (src && src->type == BL_PC) {
		...
		{
			sprintf(output,"( You Kill the %s [%s] using <%s> )", job_name(sd->status.class_), sd->status.name, ( skill_id ? skill_get_desc(skill_id) : "Melee/Reflect/Effect" ));
			clif_disp_message(&ssd->bl, output, strlen(output), SELF);
		}
		...
		&& !mapdata->flag[MF_NOEXPPENALTY] && !mapdata_flag_gvg2(mapdata))
	{
		...
		if (pc_isPremium(sd)) { // EXP penalty for VIP
...
bool pc_equipitem(struct map_session_data *sd,short n,int req_pos)
{
	...
	if (id) {
		...
		if (id->equip_script && (pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(id,sd->bl.m) || itemdb_isOnly(id,sd->bl.m)))
		...
		if(itemdb_isspecial(sd->inventory.u.items_inventory[n].card[0]))
			...
			for( i = 0; i < MAX_SLOTS; i++ ) {
				...
				if (!sd->inventory.u.items_inventory[n].card[i])
					continue;
				if ( ( data = itemdb_exists(sd->inventory.u.items_inventory[n].card[i]) ) != NULL ) {
					if (data->equip_script && (pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(data,sd->bl.m) || itemdb_isOnly(id,sd->bl.m)))
...
void pc_checkitem(struct map_session_data *sd) {
	...
	for( i = 0; i < MAX_INVENTORY; i++ ) {
		...
		if( map_getmapflag(sd->bl.m, MF_ONLY) ) {
			if( !sd->inventory_data[i]->flag.only ) {
				clif_messagecolor(&sd->bl, color_table[COLOR_RED], "Se te han desequipado objetos prohibidos en este mapa.", false, SELF);
				pc_unequipitem(sd, i, 2);
				calc_flag = 1;
				continue;
			}
		} else if( !pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) && !battle_config.allow_equip_restricted_item && itemdb_isNoEquip(sd->inventory_data[i], sd->bl.m) ) {
			pc_unequipitem(sd, i, 2);
			calc_flag = 1;
			continue;
		}
