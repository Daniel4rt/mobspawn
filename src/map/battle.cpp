...
struct Damage battle_calc_weapon_final_atk_modifiers(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	{
		// Reflect Cap Max HP [DanielArt]
		unsigned int reflect_damage = wd.damage;
		ATK_RATER(wd.damage, 50)
		reflect_damage = cap_value(wd.damage,0,tstatus->max_hp);

		status_fix_damage(target,src,reflect_damage,clif_damage(target,src,gettick(),0,0,reflect_damage,0,DMG_NORMAL,0,false),ST_REJECTSWORD);
		...
		(src->type == BL_SKILL && (skill_id == SG_SUN_WARM || skill_id == SG_MOON_WARM || skill_id == SG_STAR_WARM ))))
	{
		...
		if( rdamage > 0 ) {
			...
			if (tsc->data[SC_MAXPAIN]) {
				...
				// Reflect Shield Cap Max HP [DanielArt]
				unsigned int reflect_damage = rdamage;
				reflect_damage = cap_value(reflect_damage,0,sstatus->max_hp);
				
				battle_delay_damage(tick, wd->amotion, target, (!d_bl) ? src : d_bl, 0, CR_REFLECTSHIELD, 0, reflect_damage, ATK_DEF, rdelay ,true, false);
				skill_additional_effect(target, (!d_bl) ? src : d_bl, CR_REFLECTSHIELD, 1, BF_WEAPON|BF_SHORT|BF_NORMAL, ATK_DEF, tick);
			}
		}
	}
...
// rAthena
	...
	{ "base_exp_vip_bonus",					&battle_config.base_exp_vip_bonus,				10,		0,		500,			},
	{ "job_exp_vip_bonus",					&battle_config.job_exp_vip_bonus,				10,		0,		500,			},
	{ "quest_exp_vip_bonus",				&battle_config.quest_exp_vip_bonus,				10,		0,		500,			},
	{ "mvp_exp_vip_bonus",					&battle_config.mvp_exp_vip_bonus,				10,		0,		500,			},
	{ "drop_vip_bonus",						&battle_config.drop_vip_bonus,					10,		0,		500,			},
	{ "vip_group_level",					&battle_config.vip_group_level,					1,		0,		99,				},
