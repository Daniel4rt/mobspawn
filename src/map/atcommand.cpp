...
/*=====================================
 * Turn ON/OFF Last Teleport Ping [Easycore]
 * Usage: @lastteleport
 *-------------------------------------*/
ACMD_FUNC(lastteleport)
{
	if(sd->state.last_teleport) {
		clif_displaymessage(fd, msg_txt(sd,1549));
		sd->state.last_teleport = 0;
	} else {
		clif_displaymessage(fd, msg_txt(sd,1548));
		sd->state.last_teleport = 1;
	}
...
ACMD_FUNC(item)
{
	...
	for(j--; j>=0; j--){ //produce items in list
		...
		if (!strcmpi(command + 1, "costumeitem"))
		{
			...
			{
				clif_displaymessage(fd, "No puedes hacer Costume este item. Los Costume son solo para headgears y garments.");
				return -1;
			}
			costume = 1;
		}
...
ACMD_FUNC(heart)
{
	...
	clif_specialeffect(&sd->bl,effect,AREA);
	return 0;
}
...
ACMD_FUNC(changegm)
{
	...
	{
		clif_displaymessage(fd, msg_txt(sd,407)); // You need to be a Guild Master to use this command.
		return -1;
	}
...
ACMD_FUNC(killcounter)
{
	...
	if (strcmpi(mob, "activate") == 0) {
		...
		mob_update_variables(sd, position - 1, mob_id, 0);
