...
void initChangeTables(void)
{
	...
	set_sc( AL_PNEUMA		, SC_PNEUMA			, EFST_PNEUMA			, SCB_NONE	);
	set_sc( DC_SERVICEFORYOU		, SC_SERVICE4U			, EFST_SERVICEFORYOU			, SCB_NONE	);
	...
	set_sc( ITEM_ENCHANTARMS	, SC_ENCHANTARMS	, EFST_ENCHANTARMS		, SCB_ATK_ELE );
	...
	set_sc( GD_LEADERSHIP		, SC_LEADERSHIP		, EFST_GDSKILL_LEADERSHIP		, SCB_STR );
	set_sc( GD_GLORYWOUNDS		, SC_GLORYWOUNDS	, EFST_GDSKILL_GLORYWOUNDS		, SCB_VIT );
	set_sc( GD_SOULCOLD			, SC_SOULCOLD		, EFST_GDSKILL_SOULCOLD			, SCB_AGI );
	set_sc( GD_HAWKEYES			, SC_HAWKEYES		, EFST_GDSKILL_HAWKEYES			, SCB_DEX );
	set_sc( GD_EMERGENCYCALL	, SC_EMERGENCYCALL	, EFST_GDSKILL_EMERGENCYCALL	, SCB_NONE );
	set_sc( GD_BATTLEORDER		, SC_BATTLEORDERS	, EFST_GDSKILL_BATTLEORDER2		, SCB_STR|SCB_INT|SCB_DEX );
	set_sc( GD_REGENERATION		, SC_REGENERATION	, EFST_GDSKILL_REGENERATION2		, SCB_REGEN );
	set_sc( GD_RESTORE			, SC_RESTORE		, EFST_GDSKILL_RESTORE			, SCB_NONE );
...
static int status_get_hpbonus(struct block_list *bl, enum e_status_bonus type) {
	...
	if (type == STATUS_BONUS_FIX) {
		...
		/*//Only for BL_PC https://github.com/rathena/rathena/commit/d9ab23ae0f28905d71df43fc4047cb6bdaabbeab
		if (bl->type == BL_PC) {
			struct map_session_data *sd = map_id2sd(bl->id);
			bonus += sd->hprate;
...
int status_calc_pc_sub(struct map_session_data* sd, enum e_status_calc_opt opt)
{
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (opt&SCO_FIRST && sd->inventory_data[index]->equip_script && (pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT)
			...
			|| itemdb_isOnly(sd->inventory_data[index],sd->bl.m))) { // Execute equip-script on login
		...
		if (sd->inventory_data[index]->type == IT_WEAPON) {
			...
			if(sd->inventory_data[index]->script && (pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(sd->inventory_data[index],sd->bl.m) || itemdb_isOnly(sd->inventory_data[index],sd->bl.m))) {
	...
	if( sd->combos.count ) {
		for (i = 0; i < sd->combos.count; i++) {
			...
			while (j < combo->count) {
				...
				if (id && !pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) && (itemdb_isNoEquip(id, sd->bl.m) || !itemdb_isOnly(id, sd->bl.m))) {
					no_run = true;
					break;
				}
				j++;
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory_data[index]) {
			...
			for (j = 0; j < MAX_SLOTS; j++) { // Uses MAX_SLOTS to support Soul Bound system [Inkfish]
				...
				if(!data)
					continue;
				if (opt&SCO_FIRST && data->equip_script && (pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) || !itemdb_isNoEquip(data,sd->bl.m) || itemdb_isOnly(data,sd->bl.m))) {// Execute equip-script on login
				...
				if(!data->script)
					continue;
				if(!pc_has_permission(sd,PC_PERM_USE_ALL_EQUIPMENT) && (itemdb_isNoEquip(data,sd->bl.m) || !itemdb_isOnly(data,sd->bl.m))) // Card restriction checks.
					continue;
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory_data[index]) {
			...
			for (j = 0; j < MAX_ITEM_RDM_OPT; j++) {
				...
				if (!data || !data->script)
					continue;
				if (!pc_has_permission(sd, PC_PERM_USE_ALL_EQUIPMENT) && (itemdb_isNoEquip(sd->inventory_data[index], sd->bl.m) || !itemdb_isOnly(sd->inventory_data[index], sd->bl.m)))
					continue;
				if (i == EQI_HAND_L && sd->inventory.u.items_inventory[index].equip == EQP_HAND_L) { // Left hand status.
					sd->state.lr_flag = 1;
					run_script(data->script, 0, sd->bl.id, 0);
	...
	if (!(flag&SCSTART_NORATEDEF)) {
		...
		if(sd && SC_COMMON_MIN <= type && type <= SC_COMMON_MAX) {
			...
			if( sd->sc.data[SC_COMMONSC_RESIST] )
				rate -= 5000*sd->sc.data[SC_COMMONSC_RESIST]->val1/100;	
		}
