...
#endif
	if( test_autoloot && !status_has_mode(&md->status, MD_MVP) ) {	//Autoloot no efecto en mvps
...
void mob_killcount_status(struct map_session_data *sd, int status)
{
	sd->state.killcounter_active = (status ? true : false);
	pc_setglobalreg(sd, add_str("KC_STATUS"), status);
	return;
}
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if( sd ){
				...
				if (pc_isPremium(sd)) { // Increase item drop rate for VIP.
					drop_rate += (int)(0.5 + drop_rate * battle_config.drop_vip_bonus / 100.);
			...
			if( mvp_sd && drop_rate <= battle_config.rare_drop_announce ) {
				...
				// sprintf (message, msg_txt(NULL,541), md->name, it->jname, (float)drop_rate/100);
	...
	if(mvp_sd && md->db->mexp > 0 && !md->special_state.ai) {
		...
		if (map_getmapflag(m, MF_NOBASEEXP) || type&2)
			...
			if (count > 1) {
				...
				if(pc_isPremium(sd))
					exp += exp * (battle_config.mvp_exp_vip_bonus)/100.;
			}
		}
 ...
 *------------------------------------------*/
int mob_class_change (struct mob_data *md, int mob_id, struct block_list *usrc)
{
	unsigned int tick = gettick();
	...
	if( !usrc ) {
		...
		if( usrc->type == BL_PC ) {
			sd = (TBL_PC*)usrc;
			memcpy(md->name,sd->status.name,NAME_LENGTH);
		} else
			memcpy(md->name,md->db->jname,NAME_LENGTH);
	}
