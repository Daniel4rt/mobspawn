...
/** Read item available by mapflag (can equip item)
* Structure: <nameid>,<mode>
* @author DanielArt
*/
static bool itemdb_read_only(char* str[], int columns, int current) {
	unsigned short nameid;
	int flag;
	struct item_data *id;
	...
	if( ( id = itemdb_exists(nameid) ) == NULL )
	{
		ShowWarning("itemdb_read_only: Invalid item id %hu.\n", nameid);
		return false;
	}
...
/** Check if the item is available by item_only.txt
* @param id Item that will be checked
* @param m Map ID
* @return false: can't be used; true: can be used
* @author [DanielArt]
*/
bool itemdb_isOnly(struct item_data *id, uint16 m) {
	
	// Se define al inicio, si no, todos los mapas estarían bloqueados [DanielArt]
	struct map_data *mapdata = map_getmapdata(m);

	// Necesario que el mapa a usar para item_only tenga el mapflag MF_ONLY [DanielArt]
	if (mapdata->flag[MF_ONLY]) {
		if (!id->flag.only)
			return false;

		if ((id->flag.only&1 && !mapdata_flag_vs2(mapdata)) || // Normal
			(id->flag.only&2 && mapdata->flag[MF_PVP]) || // PVP
			(id->flag.only&4 && mapdata_flag_gvg2_no_te(mapdata)) || // GVG
			(id->flag.only&8 && mapdata->flag[MF_BATTLEGROUND]) || // Battleground
			(id->flag.only&16 && mapdata_flag_gvg2_te(mapdata)) || // WOE:TE
			(id->flag.only&(8*mapdata->zone) && mapdata->flag[MF_RESTRICTED]) // Zone restriction
			)
			return true;
	} else
		return true;
...
/**
* Check if it available to permit vending.
* @param sd
* @return True: disable vending, False: enable vending
* @author DanielArt
*/
bool itemdb_is_merchant(struct map_session_data *sd) {
	if(!sd)
		return true;
	struct item_data* id = itemdb_exists(ITEMID_MERCHANT);
	int i = 0;
	if( !id ) {
		ShowError("itemdb_is_merchant: Merchant Permit ID %d don't exist.\n",ITEMID_MERCHANT);
		return true;
	}
	
	uint16 count = 0;
	for (i = 0; i < MAX_INVENTORY; i++)
		if(sd->inventory.u.items_inventory[i].nameid == ITEMID_MERCHANT)
			count++;
	if(!count) {
		//ShowError("itemdb_is_merchant: No se ha encontrado un voucher.\n");
		return true;
	} else {
		//ShowDebug("itemdb_is_merchant: Se han encontrado '%d' x merchant voucher.\n",count);
		return false;
	}
}
...
static void itemdb_read(void) {
	...
	for(i=0; i<ARRAYLENGTH(dbsubpath); i++){
		...
		sv_readdb(dbsubpath2, "item_only.txt",          ',', 2, 2, -1, &itemdb_read_only, i > 0);
