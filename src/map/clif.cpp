...
void clif_mail_getattachment(struct map_session_data* sd, struct mail_message *msg, uint8 result, enum mail_attachment_type type) {
	...
	// Disable RODEX Mapflag [DanielArt]
	struct map_data *mapdata = map_getmapdata(sd->bl.m);
	if( mapdata->flag[MF_NORODEX]) {
		clif_displaymessage(fd,"No puedes usar RODEX en este momento.");
		result = 1;
	}
...
void clif_Mail_send(struct map_session_data* sd, enum mail_send_result result){
	...
	if( mapdata->flag[MF_NORODEX]) {
		clif_displaymessage(fd,"No puedes usar RODEX en este momento.");
		enum mail_send_result result = WRITE_MAIL_FAILED;
	}
#if PACKETVER < 20150513
...
void clif_Mail_read(struct map_session_data *sd, int mail_id)
{
	...
	struct map_data *mapdata = map_getmapdata(sd->bl.m);
	if( mapdata->flag[MF_NORODEX]) {
		clif_displaymessage(sd->fd,"No puedes usar RODEX en este momento.");
		return;
	}
...
void clif_parse_cashshop_open_request( int fd, struct map_session_data* sd ){
	if (map_getmapflag(sd->bl.m, MF_NOCASHSHOP)) {
		clif_displaymessage(fd, msg_txt(sd, 440)); // Cash Shop is disabled on this map.
		return;
	}
...
void clif_bg_memberlist(struct map_session_data *sd)
{
	...
	for (i = 0, c = 0; i < bg->count; i++)
	{
		...
		if (psd->state.bg_afk)
		{
			// Copiar solo los primeros 4 bytes de "AFK"
			memcpy(WFIFOP(fd, c * 104 + 34), "AFK", 4);
			// Rellenar el resto de los 50 bytes con ceros
			memset(WFIFOP(fd, c * 104 + 38), 0, 46);
		}
		else
		{
...
void clif_roulette_open( struct map_session_data* sd ){
	nullpo_retv( sd );
	npc_event_do_id("Settings::OnSettingCommand", sd->status.account_id);
	// nullpo_retv(sd);
