...
void script_run_autobonus(const char *autobonus, struct map_session_data *sd, unsigned int pos)
{
	...
	{
		int j;
		ARR_FIND( 0, EQI_MAX, j, sd->equip_index[j] >= 0 && sd->equip_index[j] <= EQI_MAX_BONUS && sd->inventory.u.items_inventory[sd->equip_index[j]].equip == pos );
		if( j < EQI_MAX ) {
...
static bool buildin_delitem_search(struct map_session_data* sd, struct item* it, uint8 exact_match, uint8 loc)
{
	...
	for(;;)
	{
		...
		else for( i = 0; amount && i < size; i++ )
		{
			...
			{
				char_id = MakeDWord(itm->card[2],itm->card[3]);
				if( battle_config.bg_reserved_char_id && char_id == battle_config.bg_reserved_char_id && !map_bg_items(sd->bl.m,battle_config.bg_items_on_pvp,battle_config.bg_woe_switch) )
					continue;
				if( battle_config.woe_reserved_char_id && char_id == battle_config.woe_reserved_char_id && !map_gvg_items(sd->bl.m) )
					continue;
...
BUILDIN_FUNC(getequippos) {
	...
	script_pushint( st, pc_checkequip(sd, sd->inventory.u.items_inventory[current_equip_item_index].equip) );
	return SCRIPT_CMD_SUCCESS;
}
...
BUILDIN_FUNC(getexp)
{
	...
	if(pc_isPremium(sd))
		bonus += bonus * (battle_config.quest_exp_vip_bonus) / 100.;
	if (base)
...
BUILDIN_FUNC(getmapxy)
{
	...
	x= bl->x;
	y= bl->y;
	//safestrncpy(mapname, map[bl->m].name, MAP_NAME_LENGTH); //fix https://github.com/rathena/rathena/commit/5908c0484fc3243575afbfc8f231d01f64837052
	if (bl->m >= 0)
...
BUILDIN_FUNC(awake)
{
	...
	int rid = st->rid;//https://github.com/SeravySensei/rathena/commit/9d13becb1f7373d91a4e7fabce6b3735bc2d98b3
...
/**
 * Costume an item equiped.
 * *costume <EQP_POSITION>
 * @recoded DanielArt
 **/
BUILDIN_FUNC(costume)
{
	...
	if( !script_rid2sd(sd) ) {
		ShowError("costume_script: Can't attach rid of player! (sd)\n");
		return SCRIPT_CMD_FAILURE;
	}
	if( equip_index_check(num) )
		i = pc_checkequip(sd, equip_bitmask[num]);
	if( i < 0 ) {
		ShowError("costume_script: Can't get constant of equipped item.\n");
		return SCRIPT_CMD_FAILURE;
	}
	ep = sd->inventory.u.items_inventory[i].equip;
	if( !(ep&EQP_HEAD_LOW) && !(ep&EQP_HEAD_MID) && !(ep&EQP_HEAD_TOP) && !(ep&EQP_GARMENT) ) {
		ShowError("costume_script: There's nothing equipped on player.\n");
		return SCRIPT_CMD_FAILURE;
	}
...
/*==========================================
 * Premium Char ID [DanielArt]
 *------------------------------------------*/
BUILDIN_FUNC(checkpremium)
{
	struct map_session_data *sd;
	
	int char_id = script_getnum(st,2);

	if (!script_charid2sd(char_id, sd)) {
		script_pushint(st,0);
		return SCRIPT_CMD_SUCCESS;
	}
...
BUILDIN_FUNC(branchname)
{
	struct map_session_data* sd;
	sd = map_id2sd(st->rid);
	if(!sd) {
		ShowError("buildin_func_brancname: Unknown summoner name.\n");
		script_pushconststr(st,"Desconocido");
		return SCRIPT_CMD_FAILURE;
	}
	script_pushstrcopy(st,sd->status.name);
	return SCRIPT_CMD_SUCCESS;
