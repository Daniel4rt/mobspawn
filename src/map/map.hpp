...
inline bool mapdata_bg_items(struct map_data *mapdata, bool pvp, bool bg_woe) {
	if (mapdata == nullptr)
		return false;

	if (mapdata->flag[MF_BATTLEGROUND] || mapdata->flag[MF_ALLOW_BG_ITEMS] || (pvp && (mapdata->flag[MF_PVP])))
		return true;
	
	if ((mapdata->flag[MF_GVG] || ((agit_flag || agit2_flag || agit3_flag) && mapdata->flag[MF_GVG_CASTLE])) && bg_woe)
		return true;
...
inline bool map_bg_items(int16 m, bool pvp, bool bg_woe) {
	if (m < 0)
		return false;

	struct map_data *mapdata = &map[m];

	return mapdata_bg_items(mapdata, pvp, bg_woe);
}
